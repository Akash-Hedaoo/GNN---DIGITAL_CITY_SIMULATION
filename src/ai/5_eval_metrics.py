"""
Phase 4.2: Quantitative Prediction Accuracy Evaluation
=======================================================
Measure numerical accuracy of the trained ST-GCN model
...
- Inference ONLY
- Same validation split (80/20, random_state=42)
...
OUTPUT_CSV_PATH = os.path.join(PROJECT_ROOT, "data", "processed", "datasets", "edge_error_stats.csv")
...
def compute_global_metrics(predictions, ground_truth):
...
def compute_per_edge_mae(predictions, ground_truth, idx_to_edge):
    """Compute MAE for each edge across all time samples."""
    # predictions shape: (num_samples, num_edges)
    # ground_truth shape: (num_samples, num_edges)
    
    num_edges = predictions.shape[1]
    
    edge_errors = []
    
    for edge_idx in range(num_edges):
        pred_edge = predictions[:, edge_idx]
        gt_edge = ground_truth[:, edge_idx]
        
        mae = np.mean(np.abs(pred_edge - gt_edge))
        edge_id = idx_to_edge[edge_idx]
    ...
    return pd.DataFrame(edge_errors)


def get_top_worst_edges(edge_error_df, n=10):
    """Get top N edges with highest MAE."""
    return edge_error_df.nlargest(n, 'mean_absolute_error')


def print_metrics(mae, rmse, edge_error_df):
    ...
    print("\n--- Per-Edge MAE Statistics ---")
    print(f"  Min edge MAE:  {edge_error_df['mean_absolute_error'].min():.6f}")
    print(f"  Max edge MAE:  {edge_error_df['mean_absolute_error'].max():.6f}")
    print(f"  Mean edge MAE: {edge_error_df['mean_absolute_error'].mean():.6f}")
    print(f"  Std edge MAE:  {edge_error_df['mean_absolute_error'].std():.6f}")
    
    print("\n--- Top 10 Worst Edges (Highest MAE) ---")
    top_10 = get_top_worst_edges(edge_error_df, 10)
    for i, row in enumerate(top_10.itertuples(), 1):
        print(f"  {i:2d}. {row.edge_id:<30} MAE: {row.mean_absolute_error:.6f}")
...
def main():
    print("\n" + "="*60)
    print("Phase 4.2: Quantitative Prediction Accuracy Evaluation")
    print("="*60 + "\n")
    ...
    # Compute metrics
    mae, rmse = compute_global_metrics(predictions, ground_truth)
    edge_error_df = compute_per_edge_mae(predictions, ground_truth, idx_to_edge)
    
    # Print metrics
    print_metrics(mae, rmse, edge_error_df)
    
    # Save edge error stats to CSV
    os.makedirs(os.path.dirname(OUTPUT_CSV_PATH), exist_ok=True)
    edge_error_df.to_csv(OUTPUT_CSV_PATH, index=False)
    print(f"\n[INFO] Edge error statistics saved to: {OUTPUT_CSV_PATH}")
    
    # Completion message
    print("\nPHASE 4.2 METRICS EVALUATION COMPLETE")
    print("="*60 + "\n")
