...
def normalize_coordinates(x, y):
    ...
    Pune is roughly at: 18.5204°N, 73.8567°E
    """
    # Simple linear scaling from projected to approximate lat/lon
    ...
    # Bounds from graph
    x_min, x_max = 73.82, 379410.92  # ~Easting in UTM
    y_min, y_max = 18.61, 2061677.82  # ~Northing in UTM
...
def build_graph_data_cache():
    ...
    GRAPH_DATA_CACHE = {
        ...
        'nodes': nodes[:2000],  # Limit for performance
        'edges': edges[:5000],
        'metros': metros,
        'amenities': amenities,
        'statistics': {
...
def city_data():
    ...
    if GRAPH_DATA_CACHE is None:
        # Fallback if graph can't be loaded
        return jsonify({
...
def whatif():
    ...
    # Apply scenario impact (mock for now, would use actual model)
    # Scenario types: 'road_closure', 'metro_impact', 'event', 'infrastructure'
    scenario_type = scenario.get('type', 'event')
    severity = float(scenario.get('severity', 0.5))  # 0-1
    duration = int(scenario.get('duration', 30))
    
    # Mock impact calculation
    impact_multiplier = 1.0 + (severity * 0.5)  # Up to 50% increase
